...
interface TransactionTableHaedProps {
  numSelected: number;
  onSelectAllClick: (event: React.ChangeEvent<HTMLInputElement>) => void;
  rowCount: number;
}
// テーブルヘッド
function TransactionTableHead(props: TransactionTableHaedProps) {
...
const FinancialItem = ({title, value, color}: FinancialItemProps) => {
  return(
    <Grid item xs={4} component={"span"} textAlign={"center"}>
...
export default function TransactionTable({monthlyTransactions, onDeleteTransaction}: TransactionTableProps) {
  ...
  const visibleRows = React.useMemo(
    () =>{
      const sortedMonthlyTransaction = [...monthlyTransactions].sort((a,b)=> compareDesc(parseISO(a.date),parseISO(b.date)))
